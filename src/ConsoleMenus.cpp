...
{ 
    console.setPrompt("\e[1;31mSAFE MODE >\e[0m \0"); 
}

/*==================================================================*/
void extraSafeModeConsolePrompt() 
{ 
    console.setPrompt("\e[1;31mEXTRA SAFE MODE >\e[0m \0"); 
}
...
void registerSafeModeConsoleCmds()
{
    console.registerCommand(ESP32Console::ConsoleCommandD("startethernet", [](int argc, char **argv) -> int {
        ...
        return EXIT_SUCCESS; }, PSTR("Start safe mode Ethernet for OTA or firmware updates")));
...
void registerDebuggingCmds()
{
    console.registerCommand(ConsoleCommandD("motormovetest", [](int argc, char **argv) -> int {
        if (argc != 2) 
        { 
            printf_P(PSTR("Missing 'forwards' or 'backwards'\n")); 
            return EXIT_FAILURE; 
        }
        ...
        return EXIT_SUCCESS; }, PSTR("Send debug motor movement for axis for either 'forwards' or 'backwards' moves")));
}
...
void registerConsoleCmds()
{
    if (configMode == MODE_CONTROLLER)
    {
        ...
        OptionsConsoleCommand setmotor("setmotor", [](int argc, char **argv, ParseResult result, Options options) -> int {
            ...
            if (m_index < 0) 
            { 
                printf_P(PSTR("Invalid or missing motor index (0>5)\n")); 
                return EXIT_FAILURE;
            }

            if (argc < 3) 
            { 
                printf_P(PSTR("Missing setting to update\n")); 
                return EXIT_FAILURE; 
            }
            ...
            return EXIT_SUCCESS; }, PSTR("Configure stepper motors pins and parameters"));
        ...
        console.registerCommand(ConsoleCommandD("enablemotorconfig", [](int argc, char **argv) -> int {
            if (argc != 2) 
            { 
                printf_P(PSTR("Missing 'on' or 'off'\n")); 
                return EXIT_FAILURE; 
            }
            ...
            return EXIT_SUCCESS; }, PSTR("Enable/Disable motor config at startup")));
        ...
        OptionsConsoleCommand getmotor("getmotor", [](int argc, char **argv, ParseResult result, Options options) -> int {
            ...
            return EXIT_SUCCESS; }, PSTR("Get a stepper motors current config"));
        ...
        console.registerCommand(ConsoleCommandD("stats", [](int argc, char **argv) -> int {
            ...
            return EXIT_SUCCESS; }, PSTR("Loop task communications statistics")));
    ...
    else if (configMode == MODE_CLIENT)
    {
        ...
        console.registerCommand(ConsoleCommandD("setcontrollermac", [](int argc, char **argv) -> int {
            if (argc != 2) 
            { 
                printf_P(PSTR("Missing MAC address format 00:00:00:00:00:00\n")); 
                return EXIT_FAILURE; 
            }
            ...
            return EXIT_SUCCESS; }, PSTR("Set Controller WiFI MAC address to connect to via ESP-NOW.\n 'setcontrollermac [mac_address]'\nmac_address = 00:00:00:00:00:00")));
    }
    ...
    console.registerCommand(ConsoleCommandD("mode", [](int argc, char **argv) -> int {
        if (argc != 2) 
        { 
            printf_P(PSTR("Missing 'controller' or 'client'\n")); 
            return EXIT_FAILURE; 
        }
        ...
        setConsolePrompt(PURPLE, "R!");
        return EXIT_SUCCESS; }, PSTR("Set device mode to Client or Controller. mode = 'controller' | 'client'.\nRestart required")));
    ...
    console.registerCommand(ConsoleCommandD("espnow", [](int argc, char **argv) -> int {
        ...
        {  
            arg = "off"; 
            configEspNowEnabled = false; 
        }

        NVS.setInt("ESPNOW_ENABLE", (configEspNowEnabled) ? 1 : 0);

        printf("ESP-NOW config is '%s'\r\n", arg);
        return EXIT_SUCCESS; }, PSTR("Enable/Disable ESP-NOW wireless P2P link between controller and client ESP32 devices")));
    ...
    console.registerCommand(ConsoleCommandD("log", [](int argc, char **argv) -> int {
        ...
        { 
            consoleLogging = true; 
            printf_P(PSTR("Logging enabled\n")); 
        } 
        else 
        { 
            consoleLogging = false; 
            printf_P(PSTR("Logging disabled\n")); 
        }
        return EXIT_SUCCESS; }, PSTR("Enable / Disable informational logging. ")));
    ...
    console.registerCommand(ConsoleCommandD("debug", [](int argc, char **argv) -> int {
        ...
        return EXIT_SUCCESS; }, PSTR("Enable / Disable debug logging. ")));

    console.registerCommand(ConsoleCommandD("getconfig", [](int argc, char **argv) -> int {
        readNvsConfig(false);
        return EXIT_SUCCESS; }, PSTR("Read environment configuration from NVS storage")));
    ...
    console.registerCommand(ConsoleCommandD("saveconfig", [](int argc, char **argv) -> int {
        ...
        return EXIT_FAILURE; }, PSTR("Save environment configuration to NVS storage")));

    console.registerCommand(ConsoleCommandD("resetconfig", [](int argc, char **argv) -> int {
        if (argc != 2) 
        { 
            printf_P(PSTR("Missing 'yes' parameter\n")); 
            return EXIT_FAILURE; 
        }
        ...
        //setConsolePrompt(RED, "!!");
        return EXIT_FAILURE; }, PSTR("Clear/reset NVS configuration to defaults")));
    ...
    OptionsConsoleCommand boardconf("boardconfig", [](int argc, char **argv, ParseResult result, Options options) -> int {
            ...
            if (type_index < 0 || (type_index > MAX_BOARD_TYPES) ) 
            { 
                printf("Invalid or missing board type (0>%d)\n", MAX_BOARD_TYPES); 
                return EXIT_FAILURE;
            }
            
            if (result.count("n")) 
            { 
                num_steppers = result["n"].as<int>(); 
            }

            if (num_steppers < 0 || num_steppers > 5) 
            { 
                printf_P(PSTR("Invalid number of steppers (0>6)\n")); 
                return EXIT_FAILURE;
            }
    ...
    OptionsConsoleCommand wificonf("wificonfig", [](int argc, char **argv, ParseResult result, Options options) -> int {
            ...
            if (result.count("m")) 
            { 
                mode = result["m"].as<int>(); 
            }

            if (mode < 0 || mode > 3 ) 
            { 
                printf_P(PSTR("Invalid or missing wifi mode (0>3)\n")); 
                return EXIT_FAILURE;
            }
            
            if (result.count("s")) 
            { 
                ssid = result["s"].as<std::string>(); 
            }

            if (ssid.length()  < 2) 
            { 
                printf_P(PSTR("Invalid SSID. Length must be > 2\n")); 
                return EXIT_FAILURE;
            }
            
            if (result.count("p")) 
            { 
                password = result["p"].as<std::string>(); 
            }
            ...
            return EXIT_SUCCESS; }, PSTR("Configure WiFi mode and AP/STA settings"));
    ...
    OptionsConsoleCommand inputconf("inputconfig", [](int argc, char **argv, ParseResult result, Options options) -> int {
        ...
        if (i_index < 0) 
        { 
            printf_P(PSTR("Invalid or missing input index (0>6)\n")); 
            return EXIT_FAILURE;
        }
        ...
        return EXIT_SUCCESS; }, PSTR("Configure Input pins and parameters"));
    ...
    OptionsConsoleCommand outputconf("outputconfig", [](int argc, char **argv, ParseResult result, Options options) -> int {
        ...
        if (i_index < 0) 
        { 
            printf_P(PSTR("Invalid or missing output index (0>6)\n")); 
            return EXIT_FAILURE;
        }
        ...
        return EXIT_SUCCESS; }, PSTR("Configure Output pins and parameters"));
    ...
    OptionsConsoleCommand spiethconf("spiethconfig", [](int argc, char **argv, ParseResult result, Options options) -> int {
            ...
            { 
                printf_P(PSTR("Invalid or missing MISO pin (0>128)\n")); 
                return EXIT_FAILURE;
            }
            ...
            { 
                printf_P(PSTR("Invalid or missing MOSI pin (0>128)\n")); 
                return EXIT_FAILURE;
            }
            ...
            { 
                printf_P(PSTR("Invalid or missing SCK pin (0>128)\n")); 
                return EXIT_FAILURE;
            }
            ...
            { 
                printf_P(PSTR("Invalid or missing Chip Select pin (0>128)\n")); 
                return EXIT_FAILURE;
            }
            ...
            { 
                printf_P(PSTR("Invalid or missing Interrupt pin (0>128)\n")); 
                return EXIT_FAILURE;
            }
            ...
            return EXIT_SUCCESS; }, PSTR("SPI Ethernet Pin Configuration."));
