        ...
        if (xQueueReceive(inputInterruptQueue, &inputIndex, portMAX_DELAY))
        {
            ...
            if ((fb.io & io_mask) != registerBitState)
            {
                ...
                if (configMode == MODE_CONTROLLER)
                {
                    if (!motorsMoving) 
                    {
                        // Only send a udp feedback pkt if motors are stationary. When moving, feedback sent in realtime as per the fb.io struct
...
/*================= Setup sections =================================*/
/*==================================================================*/
void setupInputPin(int inputIndex)
{
    /* See Config.h for inputPin board specific configurations */
    const inputpin_config_t *pin_config = &board_pin_config.inputConfigs[inputIndex];
...
{
    /* One-shot background WiFi connection task */
...
{
    /* Anything requiring exec on Core0 should be setup in this one-shot task */
...
/*==================================================================*/
bool safeModeHandler() 
{
    /* Returns true if in safemode. Setup() exits if true */
...
void setup()
{
...
#ifndef ESP32_SPI_ETHERNET
    // Only do this once when not SPI ethernet. SPI ethernet driver installs this automatically
    ...
    while (!startupStage1Complete)
    { 
        // Wait for all required tasks to finish starting before enabling console
        delay(200);
    }
